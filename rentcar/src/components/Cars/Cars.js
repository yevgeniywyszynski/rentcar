  ...
  useEffect(() => {
    if (search.searchActive) {
      const getSearchRequest = async () => {
        let resultSearchRequest = await axios.get(
          `http://localhost:8000/search?keyword=${search.keyword}&type=${search.typeCar}&rating=${search.rating}`
        );
        ...
        {carsToShow.map((cars) => (
          <div key={uuidv4()} className={styles.cars}>
            ...
            <div className={styles.carDetailsWrapper}>
              <Link to={`/booking/${cars.idCar}`} className={styles.btnBooking}>
                BOOK NOW
              </Link>
