...
export const nextCarToShow = ({ carsToShow, search }) => {
  ...
  if (search.searchActive) {
    result = result.filter((e) =>
      e.model.startsWith(search.keyword.toUpperCase())
    );
    result = result.filter((e) =>
      e.carType.startsWith(search.typeCar.toLowerCase())
    );
...
export const loadNextCarToShow = (payload) => ({
  payload,
  type: LOAD_NEXTCARTOSHOW,
});
...
export const requestNextCarListToShow = (pageId) => {
  return async (dispatch) => {
    ...
    try {
      let carsToShow = await axios.get(
        `http://localhost:8000/current?page=${pageId}`
      );
      ...
      console.log(e);
      dispatch(
        errorRequest({ name: "ERROR_REQUEST", error: "could not fetch data" })
      );
    }
  };
};
...
export default function reducer(statePart = [], action = {}) {
  switch (action.type) {
    case LOAD_NEXTCARTOSHOW:
      ...
      console.log("requestCar", requestCars);
      ...
      return {
        ...
        isNext: action.payload.info.next ? true : false,
      }; //uzupelniamy tablice z cars nastepnymi z requestu
    case GET_ALLCARS:
